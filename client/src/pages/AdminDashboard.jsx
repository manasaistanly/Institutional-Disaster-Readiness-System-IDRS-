...
import { FaBullhorn, FaCheckCircle, FaExclamationCircle, FaTrash, FaShieldAlt, FaUsers, FaHistory, FaSignal, FaBrain, FaMapMarkerAlt } from 'react-icons/fa';
...
const AdminDashboard = () => {
    ...
    const [targetScope, setTargetScope] = useState('global'); // 'global', 'state', 'district'
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            // Build targetRegions object
            const targetRegions = targetScope === 'global'
        ...
        <div className="min-h-screen pt-4 pb-8 px-4 container mx-auto text-white">
            ...
            <div className="grid lg:grid-cols-12 gap-6 h-full">
                    ...
                    <div className="bg-[#1f2937] border border-white/5 rounded-3xl p-6 shadow-xl">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="border-t border-white/10 pt-5">
                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-3">Target Region</label>
                                ...
                                <div className="grid grid-cols-3 gap-2 mb-4">
                                    <button
                                        ...
                                        className={`py-2 px-3 rounded-lg text-xs font-bold transition-all ${targetScope === 'global'
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setTargetScope('state')}
                                        className={`py-2 px-3 rounded-lg text-xs font-bold transition-all ${targetScope === 'state'
                                                ? 'bg-blue-600 text-white'
                                                : 'bg-white/5 text-gray-400 hover:bg-white/10'
                                            }`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`py-2 px-3 rounded-lg text-xs font-bold transition-all ${targetScope === 'district'
                                                ? 'bg-blue-600 text-white'
                                                : 'bg-white/5 text-gray-400 hover:bg-white/10'
                                            }`}
                                    >
                                ...
                                {targetScope !== 'global' && (
                                    <div className="space-y-3">
                                        <div>
                                            <label className="block text-xs text-gray-400 mb-2">Select State(s)</label>
                                            <select
                                                multiple
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (values.length === 1) {
                                                        setAvailableDistricts(getDistrictsForState(values[0]));
                                                    }
                                                }}
                                                className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-white text-sm max-h-24 outline-none focus:border-blue-500"
                                            >
                                        ...
                                        {/* District Selection */}
                                        {targetScope === 'district' && selectedStates.length === 1 && (
                                            <div>
                                                <label className="block text-xs text-gray-400 mb-2">Select District(s)</label>
                                                <select
                                                    multiple
                                                ...
                                                <p className="text-xs text-gray-600 mt-1">Targeting: {selectedDistricts.join(', ') || 'None'}</p>
                                            </div>
                                        )}
                                    </div>
                                )}
                ...
                <div className="lg:col-span-5 h-full">
                    ...
                    {/* Quick Action Widget */}
                    <div className="bg-[#1f2937] border border-white/5 rounded-3xl p-6 shadow-xl mt-6">
                        <div className="flex items-center gap-4 mb-4">
                            <div className="w-10 h-10 bg-purple-600/20 rounded-lg flex items-center justify-center text-purple-400">
                                <FaBrain />
                            </div>
                            <div>
                                <h3 className="font-bold text-white">Training Modules</h3>
                                <p className="text-xs text-gray-400">Create quizzes for users</p>
                            </div>
                        </div>
                        <button
                            onClick={() => window.location.href = '/admin/quiz'}
                            className="w-full py-3 bg-white/5 hover:bg-purple-600 hover:text-white rounded-xl border border-white/10 transition-all font-bold text-sm"
                        >
                            Manage Quizzes
                        </button>
                    </div>
